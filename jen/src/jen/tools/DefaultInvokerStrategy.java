...
import jen.methods.SuperConstructor;
    ...
    */
    ...
    static final class DefaultProxyConstructor extends GeneratedSoftMethod {
		...
		protected void generateCode(GeneratorAdapter gen) {
            ...
            for (int i = 1; i < this.getArgumentTypes().size(); i++) {
                gen.loadArg(i);
                Type xType = this.getArgumentTypes().get(i);
                xSignatureType.append(xType.getClassName());
                if (i < this.getArgumentTypes().size() - 1) {
                    xSignatureType.append(",");
                }
            }
            xSignatureType.append(")");
            gen.invokeConstructor(Type.getType(binaryToDescriptor(this.getSoftClass().getSuperClassInternalName())),
                    org.objectweb.asm.commons.Method.getMethod(xSignatureType.toString()));
            gen.loadArg(0);
            gen.putField(this.getSoftClass().getType(), INV_HANDLER_FIELD, Type.getType(InvocationHandler.class));
            gen.returnValue();
            gen.endMethod();
        }
    }
    ...
    static final class DefaultSuperInvoker extends GeneratedSoftMethod {
        ...
        DefaultSuperInvoker(SoftClass softClass, Method m) {
            ...
            this.superType = Type.getType(binaryToDescriptor(this.getSoftClass().getSuperClassInternalName()));
		...
		protected void generateCode(GeneratorAdapter gen) {
            ...
            gen.loadThis();
            if (this.withParams) gen.loadArgs();
            gen.invokeConstructor(this.superType, this.superMethod);
            gen.returnValue();
            gen.endMethod();
        }
    }
    ...
    static final class DefaultInvokerDispatcher extends GeneratedSoftMethod {
        ...
        DefaultInvokerDispatcher(SoftClass softClass, Method m) {
            super(softClass, Opcodes.ACC_PUBLIC,
		...
		protected void generateCode(GeneratorAdapter gen) {
            ...
            gen.loadThis();
            gen.dup();                                                                                               // this .. this
            gen.getField(this.getSoftClass().getType(), "__inv", INV_HANDLER_TYPE);                                         // this .. InvHandler
            gen.loadThis();                                                                                          // this .. InvHandler .. this
            gen.getStatic(this.getSoftClass().getType(), SoftProxy.methodFieldName(this.methodDesc), METHOD_TYPE);               // this .. InvHandler .. this .. Method
            ...
            if (this.retType != Type.VOID_TYPE) {
                ...
                if (retDesc.startsWith("L") || retDesc.startsWith("[")) {
                    ...
                    defaultPrimitiveReturn(primitiveTypeIdent(this.retType), gen);                                             // xCONST_0
                    gen.goTo(retInsn);

                    gen.mark(nullPushSkip);
                    gen.unbox(this.retType);                                                                                  // retval (or xCONST_0)
                }
            }
    ...
    */
    ...
    public static final Object invokeSuper(Object proxy, Method m, Object[] args) throws Throwable {
      ...
      if (isDefaultProxyClass(c)) {
        try {
          return knownMethod(c,superInvokerName(m), m.getParameterTypes()).invoke(proxy,args);
